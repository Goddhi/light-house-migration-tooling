...
export class LighthouseClient {
  ...
  // Simple API key sanity check – you can swap to a more specific endpoint if you like
  async validateApiKey() {
    try {
      await lighthouse.getBalance(this.apiKey); // returns { data: { dataLimit, dataUsed } }
      return true;
    } catch {
      return false;
    }
  ...
  // Optional: return fake zeros if you don’t have a quota API
  async getStorageInfo() {
    const { data } = await lighthouse.getBalance(this.apiKey);
    const dataUsed = Number(data?.dataUsed || 0);
    const dataLimit = Number(data?.dataLimit || 0);
    return {
      dataUsed,
      dataLimit,
      utilizationPercent: dataLimit ? (dataUsed / dataLimit) * 100 : 0,
    };
  }

  async checkStorageQuota(bytesNeeded) {
    const { data } = await lighthouse.getBalance(this.apiKey);
    const used = Number(data?.dataUsed || 0);
    const limit = Number(data?.dataLimit || 0);
    const available = Math.max(0, limit - used);
    return {
      sufficient: available >= bytesNeeded,
   ...
   * @param {(pct:number, raw:any)=>void} onProgress
   * @returns {{ cid: string, size?: number }}
   */  async uploadBuffer(buffer /* _fileName, _mimeType, onProgress (ignored) */) {
    try {
      const resp = await lighthouse.uploadBuffer(buffer, this.apiKey);
      const d = resp?.data || {};
      const cid = d.cid || d.Hash;
      const size = Number(d.Size) || buffer.length;
      if (!cid) throw new Error('Upload failed: no CID returned');
      return { cid, size };
    } catch (err) {
      throw new Error(`Upload to Lighthouse failed: ${err.message}`);
    }
  }
}